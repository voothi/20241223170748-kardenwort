...
LinguaCard Creator is a powerful command-line utility designed to accelerate language learning by automatically creating Anki flashcards from any text. It intelligently processes your source material, extracts vocabulary (tokens) and full sentences, and imports them directly into Anki for you.
...
-   [**Anki Template (20241106211123-anki-template)**](https://github.com/user/repo-link) (*link placeholder*): A specialized Anki card template required for the generated TSV files to display correctly. The generated TSV has many fields that map to this template.
...
1.  **Clone the repositories**:
    ...
    git clone <url-for-anki-template-repo> # (Or import the template manually)
    ```
...
6.  **Import the Anki Template**:
    Import the note type from the `20241106211123-anki-template` project into your Anki profile.
...
3.  **Run a batch script**:
    ...
    -   **To create English vocabulary cards from a single text file:**
        ...
        ```
    -   **To create German cards from three text files:**
        ```bash
        t_batch_de.triple.bat
        ```
