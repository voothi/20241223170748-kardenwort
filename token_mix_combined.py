...
def get_corrected_lemma(token, german_dict, fix_genitive_flag=False):
    ...
    if (fix_genitive_flag and 
        ...
        # If the lemma ends with 's', try removing it and check if the result is a valid word.
        # This handles cases like "Hauses" -> "Haus" or "Gebirges" -> "Gebirge".
        if spacy_lemma.endswith('s') and len(spacy_lemma) > 1:
            candidate_lemma = spacy_lemma[:-1]
            # Check against dictionary to avoid over-correction (e.g., "Bus" -> "Bu")
...
def process_sentence_lemmas(sentence, lemma_index, nlp, german_dict, gcs=False, ahocs=None, gcs_in_wordlist=False, gcs_only_nouns=True, make_singular=False, no_make_singular=False, gcs_combine_noun_modes=False, gcs_fix_genitive=False):
    doc = nlp(sentence)
    final_tokens = set()
...
def process_text_v1(
    ...
    gcs_only_nouns=True, make_singular=False, no_make_singular=False, gcs_combine_noun_modes=False, gcs_fix_genitive=False
):
        ...
        with open(output_file, "w", newline="", encoding="utf-8") as tsvfile:
            ...
            for token in sorted_tokens:
                ...
                if include_simple_list:
                    lemmas = process_sentence_lemmas(l1_sentence, lemma_index, nlp, german_dict, gcs, ahocs, gcs_in_wordlist, gcs_only_nouns, make_singular, no_make_singular, gcs_combine_noun_modes, gcs_fix_genitive)
